...
- [ ] **Canal Global:** Os usuários serão conectados automaticamente ao canal global assim que o apelido for definido.
- [ ] **Notificação de Presença:** O sistema deve exibir uma mensagem automática para todos os usuários informando quando um novo participante entrou ou saiu do chat.

### MENSAGERIA EM TEMPO REAL:
- [ ] **Envio de mensagens:** Usuários devem poder redigir e enviar mensagens de texto.
- [ ] **Recebimento de mensagens:** A interface de todos os usuários conectados deve ser atualizada com a nova mensagem recebida, sem atualizar a página.
- [ ] **Identificação de autoria:** Cada mensagem exibida deve estar acompanhada do apelido do remetente e do horário do envio (timestamp).

### INTERFACE E FEEDBACK:
- [ ] **Histórico Local Volátil:** O sistema deve manter o histórico das mensagens enviadas e recebidas durante a sessão atual (enquanto a aba estiver aberta).
- [ ] **Limpeza de Input:** O sistema deve limpar o campo de digitação imediatamente após o envio bem-sucedido de uma mensagem.
